...
func (a *App) startup(ctx context.Context) {
	...
	// Disable ggml-metal residency sets to prevent GGML_ASSERT([rsets->data count] == 0)
	// SIGABRT panics when the app terminates or reloads with long Dictation prompts.
	os.Setenv("GGML_METAL_NO_RESIDENCY", "1")
...
func (a *App) onHotkeyTriggered() {
	...
	if a.audio.IsRecording() {
		...
		go func() {
			...
			if len(pcm) > 0 {
				select {
				case a.whisperCh <- TranscriptionJob{PCM: pcm, Prompt: " Here is a clean, grammatically correct transcript without filler words or stutters:"}:
					log.Printf("audio: %d samples queued for transcription (suppress instruction included)", len(pcm))
				default:
